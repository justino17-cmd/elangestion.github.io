...
Environnement :
- **Chrome DevTools MCP : tu ne l'as pas, et ce n'est pas une erreur de configuration.**
  Ta ligne `tools:` est une liste explicite, et une liste explicite ferme l'accès à TOUS les
  outils MCP — elle retire au passage `ToolSearch`, sans lequel ils restent inatteignables même
  écrits noir sur blanc. Éprouvé quatre fois le 8 septembre 2026 (voir CLAUDE.md). N'essaie pas,
  ne le réclame pas : mesure avec Playwright ci-dessous. Une vraie trace de performance se prend
  depuis la session principale, qui, elle, y a accès.
  ⛔ Et quand elle la prend : **bêta uniquement** — le serveur MCP expose tout le contenu de la
  page ; jamais sur `app.html` avec une session client ouverte (noms et adresses de vrais clients).
